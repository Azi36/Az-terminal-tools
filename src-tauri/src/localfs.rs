...
pub fn local_touch(path: String) -> Result<(), SshError> {
    ...
    std::fs::write(&path, b"").map_err(|e| SshError::new("建不了文件，检查权限", e))
}
...
pub fn local_read_text(path: String, encoding: Option<String>) -> Result<TextFile, SshError> {
    ...
    Ok(TextFile {
        ...
        newline: crate::sftp::newline_of(&content),
        content,
        size: meta.len(),
        mode: None,
        mtime,
...
pub fn local_write_text(
    ...
    // 也别把 CRLF 的文件整篇换成 LF
    let text = crate::sftp::restore_newlines(&content, newline.as_deref());
    ...
    std::fs::write(&tmp, &bytes).map_err(|e| SshError::new("写不进去，检查权限或只读属性", e))?;
